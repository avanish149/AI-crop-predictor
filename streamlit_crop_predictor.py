...
if "rand_values" not in st.session_state:
    st.session_state["rand_values"] = {
        "N": 50, "P": 50, "K": 50, "temperature": 25.0, "humidity": 50.0, "ph": 7.0, "rainfall": 100.0
    }
...
with st.form("prediction_form"):
    N = st.number_input("Nitrogen (N)", min_value=0, max_value=200, value=st.session_state["rand_values"]["N"])
    P = st.number_input("Phosphorus (P)", min_value=0, max_value=200, value=st.session_state["rand_values"]["P"])
    K = st.number_input("Potassium (K)", min_value=0, max_value=200, value=st.session_state["rand_values"]["K"])
    temperature = st.number_input("Temperature (°C)", min_value=0.0, max_value=60.0, value=st.session_state["rand_values"]["temperature"])
    humidity = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=st.session_state["rand_values"]["humidity"])
    ph = st.number_input("pH Value", min_value=0.0, max_value=14.0, value=st.session_state["rand_values"]["ph"])
    rainfall = st.number_input("Rainfall (mm)", min_value=0.0, max_value=400.0, value=st.session_state["rand_values"]["rainfall"])
